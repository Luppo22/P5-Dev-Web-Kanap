...
function getCart() {
    if (userProducts === null || userProducts == 0) {
        ...
        userProducts.forEach(productItem => {
            ...
            fetch(`http://127.0.0.1:3000/api/products/${productItem.id}`,
                { method: 'GET' })
...
function isOrderValid() {
    let isValid = true;
    // Parcourir les produits et vérifier que la quantité est supérieure à 0 et inférieure 100
    let quantityInput = document.querySelectorAll(".itemQuantity");

    quantityInput.forEach(productItem => {
        console.log(productItem.value)
        if (parseInt(productItem.value) <= 0 || parseInt(productItem.value) > 100) {
            alert("Veuillez donner une quantité inférieure ou égale à 0 et/ou supérieure à 100");
            isValid = false;
        }
    });
    return isValid;
...
if (userProducts) {
    ...
    alert('Vous ne pouvez pas passer une commande vide !')
}